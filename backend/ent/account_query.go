...
func (_q *AccountQuery) QueryOpenaiWebThreads() *OpenAIWebThreadQuery {
...
func (_q *AccountQuery) WithOpenaiWebThreads(opts ...func(*OpenAIWebThreadQuery)) *AccountQuery {
	query := (&OpenAIWebThreadClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withOpenaiWebThreads; query != nil {
		if err := _q.loadOpenaiWebThreads(ctx, query, nodes,
			func(n *Account) { n.Edges.OpenaiWebThreads = []*OpenAIWebThread{} },
			func(n *Account, e *OpenAIWebThread) { n.Edges.OpenaiWebThreads = append(n.Edges.OpenaiWebThreads, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadOpenaiWebThreads(ctx context.Context, query *OpenAIWebThreadQuery, nodes []*Account, init func(*Account), assign func(*Account, *OpenAIWebThread)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(openaiwebthread.FieldAccountID)
	}
	query.Where(predicate.OpenAIWebThread(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.OpenaiWebThreadsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
