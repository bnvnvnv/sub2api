...
// The OpenAIWebThreadFunc type is an adapter to allow the use of ordinary function as a Querier.
type OpenAIWebThreadFunc func(context.Context, *ent.OpenAIWebThreadQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f OpenAIWebThreadFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.OpenAIWebThreadQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.OpenAIWebThreadQuery", q)
}

// The TraverseOpenAIWebThread type is an adapter to allow the use of ordinary function as Traverser.
type TraverseOpenAIWebThread func(context.Context, *ent.OpenAIWebThreadQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseOpenAIWebThread) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseOpenAIWebThread) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.OpenAIWebThreadQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.OpenAIWebThreadQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.OpenAIWebThreadQuery:
		return &query[*ent.OpenAIWebThreadQuery, predicate.OpenAIWebThread, openaiwebthread.OrderOption]{typ: ent.TypeOpenAIWebThread, tq: q}, nil
