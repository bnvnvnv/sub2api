export default {
  ...
  openAIWeb: {
    title: 'OpenAI Web 会话',
    description: '面向 OpenAI 网页会话的页面级工作台，用来管理线程归属、粘性会话和元数据查看，同时保持现有 API 与 Codex 转发链路不变。',
    summaryBadge: 'Page-Level MVP',
    eligibleGroups: '可用分组',
    activeThreads: '活动线程',
    proReadyGroups: 'Pro 就绪分组',
    privacyTitle: '服务端只保存线程元数据',
    privacyDesc: '当前 MVP 默认只在服务端保存线程绑定信息、上游会话 ID 和调度归属，消息正文后续优先走浏览器本地缓存。',
    scopeTitle: '先补工作台，不改转发链路',
    scopeDesc: '这个页面复用现有的选号、粘性会话和账号池逻辑，只把用户侧的线程工作台补完整，不会改掉当前转发行为。',
    noEligibleTitle: '暂无可用的 OpenAI Web 权限',
    noEligibleDesc: '至少需要一个当前可访问的 OpenAI 分组，且分组里存在可用于网页会话的 OAuth 账号。现在公开 standard 分组也会计入，subscription 分组仍然要求有效订阅。',
    createTitle: '新建网页线程',
    createDesc: '线程创建后会绑定到当前用户，并复用现有分组选号与粘性会话逻辑。',
    createUnavailableTitle: '当前无法新建线程',
    createUnavailableDesc: '你仍然可以查看已有线程元数据，但目前没有可用于新建 OpenAI Web 线程的分组。',
    refresh: '刷新',
    groupLabel: '可用分组',
    noGroupDesc: '暂无分组描述',
    defaultModelTag: '默认模型 {model}',
    proAccountsTag: '含 Pro 能力账号',
    subscriptionEndTag: '{date} 到期',
    titleLabel: '线程标题',
    titlePlaceholder: '例如：产品方案讨论 / 图像提示词草稿',
    modelLabel: '请求模型',
    modelAuto: '自动使用默认模型',
    cachePolicyLabel: '缓存策略',
    cachePolicyHint: '服务端始终只保存线程元数据。当前消息正文只保存在本浏览器，local_encrypted 先保留策略位，前端本地加密会在后续补上。',
    createButton: '创建线程',
    creating: '创建中...',
    threadListTitle: '现有线程',
    threadListDesc: '在这里检索、筛选并查看当前账号池已经绑定的线程记录。',
    searchPlaceholder: '按标题、分组、模型、账号或线程 ID 搜索',
    filterAll: '全部',
    filterReady: '稳定',
    filterAttention: '需关注',
    emptyTitle: '还没有线程',
    emptyDesc: '先从左侧创建一个线程，后续完整聊天工作台会直接复用这些绑定信息。',
    emptySearchTitle: '没有匹配的线程',
    emptySearchDesc: '调整搜索词或筛选条件后再试。',
    selectedThreadBadge: '当前线程',
    selectedThreadDesc: '这里展示当前线程对应的上游会话标识、调度状态和本地缓存策略。',
    selectThreadTitle: '先选择一个线程',
    selectThreadDesc: '从左侧选择一个线程后，这里会展示它的路由细节、上游标识和粘性会话状态。',
    groupInfo: '分组',
    accountInfo: '账号',
    planInfo: '账号套餐',
    providerInfo: '提供方',
    syncStatusInfo: '同步状态',
    historyModeInfo: '历史模式',
    cachePolicyInfo: '缓存策略',
    capabilityInfo: '能力',
    threadMetaTitle: '线程元数据',
    sessionIdentifiersTitle: '会话标识',
    localThreadId: '本地线程 ID',
    pageSessionId: '页面会话 ID',
    upstreamConversationId: '上游对话 ID',
    upstreamSessionId: '上游续写节点 ID',
    pendingBinding: '等待上游绑定',
    copyLocalId: '复制本地 ID',
    ...
    modelAutoResolved: '按默认模型解析',
    threadNeedsAttention: '需要关注',
    sessionCanvasTitle: '会话画布',
    sessionCanvasDesc: '当前后端还没有开放完整消息体接口，所以这里先围绕已经具备的线程状态来展示工作台。',
    sessionSystemBubble: '该线程已固定到分组 {group}，当前请求模型为 {model}。',
    sessionStorageBubble: '服务端当前只保存线程元数据、粘性会话归属和上游标识。消息正文后续优先留在浏览器本地缓存或上游网页历史中。',
    sessionPendingBubble: '这个线程还没有生成真实的上游对话绑定，等网页侧聊天流量接入后会自动补齐。',
    sessionBoundBubble: '该线程已经绑定上游对话：{id}',
    chatTitle: '聊天面板',
    chatDesc: '消息正文只缓存在当前浏览器，本次请求的路由和计费仍然接入现有后端链路。',
    messageCount: '{count} 条消息',
    emptyChatTitle: '先发起第一轮对话',
    emptyChatDesc: '这个线程当前归属分组 {group}，会使用 {model}。发送第一条消息后会绑定上游网页会话，并开始在当前浏览器缓存消息。',
    chatModelLabel: '聊天模型',
    reasoningEffortLabel: '思考强度',
    reasoningEffortAuto: '跟随默认',
    chatBillingHint: '当前线程已接入现有 usage / billing 计费体系',
    composerPlaceholder: '在这里输入文本，也可以附带图片。回车发送，Shift+Enter 换行。',
    composerHint: '支持文本和图片混合发送；模型切换会更新当前线程默认模型，思考强度保存在当前浏览器，换设备不会自动同步。',
    addImages: '添加图片',
    clearImages: '清空图片',
    removeImage: '移除',
    ...
    attachmentTypeInvalid: '只能上传图片文件。',
    attachmentTooLarge: '单张图片不能超过 {size}。',
    attachmentReadFailed: '读取图片失败，请重试。',
    send: '发送',
    sending: '发送中...',
    userLabel: '你',
    assistantLabel: '助手',
    ...
    requestIdTag: '请求 {id}',
    responseIdTag: '响应 {id}',
    usageTag: '输入 {input} · 输出 {output} · 合计 {total}',
    localCacheImageWarning: '浏览器本地缓存空间不足，已仅保留文本消息；本轮生成图片刷新后不会继续保留。',
    localCacheDisabledWarning: '浏览器本地缓存写入失败，当前消息仅保留在本次页面内存中。',
    timelineTitle: '时间线',
    timelineCreated: '线程已创建',
    timelineUpdated: '最近更新',
    timelineSynced: '最近同步快照',
    timelineCacheMode: '本地缓存模式',
    timelineNotSynced: '尚无同步快照',
    failedToLoad: '加载 OpenAI Web 页面数据失败',
    createSuccess: '线程创建成功',
    createFailed: '线程创建失败',
    archiveSuccess: '线程已归档',
    archiveFailed: '线程归档失败',
    sendFailed: '发送消息失败',
    status: {
      active: '活动中',
      archived: '已归档',
    ...
    cachePolicy: {
      local_only: '仅浏览器本地',
      local_encrypted: '浏览器本地（加密待补）'
    },
    accessMode: {
      standard: '公开 standard 访问',
      subscription: '订阅访问'
    }
  },
