export default {
  ...
  openAIWeb: {
    title: 'OpenAI Web 会话',
    description: '面向网页会话的最小管理入口。当前阶段只管理可用订阅、线程绑定和粘性会话，不改动现有 API/Codex 转发链路。',
    summaryBadge: 'Page-Level MVP',
    eligibleGroups: '可用分组',
    activeThreads: '活动线程',
    privacyTitle: '服务端只保存线程元数据',
    privacyDesc: '当前 MVP 默认只在服务端保存线程绑定信息、上游会话 ID 和调度归属，消息正文后续优先走浏览器本地缓存。',
    scopeTitle: '当前页面先做线程管理',
    scopeDesc: '这里先提供权限探测、线程创建和归档入口。真实聊天面板会在后续阶段复用这些线程元数据接入。',
    noEligibleTitle: '暂无可用的 OpenAI Web 权限',
    noEligibleDesc: '需要至少一个有效的 OpenAI 订阅分组，且该分组里存在可用于网页会话的 OAuth 账号。',
    createTitle: '新建网页线程',
    ...
    threadListDesc: '每个线程都和当前用户隔离，并绑定一个上游账号与 page session。',
    emptyTitle: '还没有线程',
    emptyDesc: '先从左侧创建一个线程，后续聊天 UI 会直接复用这些线程绑定。',
    groupInfo: '分组',
    accountInfo: '账号',
    capabilityInfo: '能力',
    updatedAt: '最近更新',
